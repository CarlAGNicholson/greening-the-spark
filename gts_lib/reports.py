...
def getEfficiencyScore(cumulative_demand, cumulative_surplus, cumulative_shortfall, capacity, storage_discrepancy):  
    DEBUG = False
    ...
    storage_discrepancy_fraction = storage_discrepancy/capacity
    if DEBUG: print("§ reports storage_discrepancy_fraction:", storage_discrepancy_fraction)
...
def printSimulationReport(start_time, end_time, run_time, StaticModels, DynamicModels, StorageModels, GTSGrid, ParameterTypes, difficulty):
    ...
    initial_stored = ParameterTypes["initial_conditions"].parameters["batteries.level"] * ParameterTypes["scale_factors"].parameters["batteries.capacity"]+ \
                     ParameterTypes["initial_conditions"].parameters["hydro.level"] * ParameterTypes["scale_factors"].parameters["hydro.capacity"]
    if DEBUG: print("§ reports initial stored", initial_stored)
    ...
    final_stored = StorageModels["batteries"].level + \
                   StorageModels["hydro"].level
    if DEBUG: print("§ reports final stored", final_stored)
    ...
    storage_discrepancy = abs(final_stored - initial_stored)
    
    efficiency_score, surplus_comment, shortfall_comment, storage_comment = getEfficiencyScore(cumulative_demand, cumulative_surplus, cumulative_shortfall, capacity, storage_discrepancy)
    ...
    print(" Initial stored\t\t" + f"{initial_stored:,.2f}")
